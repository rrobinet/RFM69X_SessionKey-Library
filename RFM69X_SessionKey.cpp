// Date: 02/10/2017
// This version RFM69X_Session eXtended for compatibility with ESP32 RFM69X library and is derived from the previous one of
// the 11/10/2016.
// It is adapted to compute SPI transfer while exchanging Keys through the new RFM69X function interruptHandling() 
// It still based one the original one of:
// https://github.com/dewoodruff/RFM69X_SessionKey (February 2015)
...
//  12. Correct typo in RFM69X_SessionKey::initialise instead of RFM69X_SessionKey::initialize
//  13. Improve messages of 9.
//	14. Correct restore Interrupt in receiveDone() function for ESP8266 compatibilities 
//  15. Use interruptHandling in RFM69X_SessionKey::sendWithSession and in RFM69X_SessionKey::receiveDone
// **********************************************************************************
...
#include <RFM69X_SessionKey.h>
#include <RFM69X.h>   // include the RFM69 library files as well
#include <RFM69Xregisters.h>
#include <SPI.h>

volatile uint8_t RFM69X_SessionKey::SESSION_KEY_INCLUDED; // flag in CTL byte indicating this packet includes a session key
volatile uint8_t RFM69X_SessionKey::SESSION_KEY_REQUESTED; // flag in CTL byte indicating this packet is a request for a session key
volatile uint8_t RFM69X_SessionKey::SESSION_KEY_RCV_STATUS;//!RVDB add a variable to indicate the type the session key status after receive was done

volatile unsigned long RFM69X_SessionKey::SESSION_KEY; // !RVDB set to the session key value for a particular transmission
volatile uint8_t RFM69X_SessionKey::SESSION_KEY1; // !RVDB set to the session key High value for a particular transmission
volatile uint8_t RFM69X_SessionKey::SESSION_KEY2; // !RVDB set to the session key Medium value for a particular transmission
volatile uint8_t RFM69X_SessionKey::SESSION_KEY3; // !RVDB set to the session key Medium value for a particular transmission
volatile uint8_t RFM69X_SessionKey::SESSION_KEY4; // !RVDB set to the session key Low value for a particular transmission
volatile unsigned long RFM69X_SessionKey::INCOMING_SESSION_KEY; // !RVDB set on an incoming packet and used to decide if receiveDone should be true and data should be processed
volatile uint8_t RFM69X_SessionKey::INCOMING_SESSION_KEY1; // !RVDB set to the session key High value for a particular transmission
volatile uint8_t RFM69X_SessionKey::INCOMING_SESSION_KEY2; // !RVDB set to the session key Medium value for a particular transmission
volatile uint8_t RFM69X_SessionKey::INCOMING_SESSION_KEY3; // !RVDB set to the session key Medium value for a particular transmission
volatile uint8_t RFM69X_SessionKey::INCOMING_SESSION_KEY4; // !RVDB set to the session key Low value for a particular transmission
volatile uint16_t RFM69X_SessionKey::_waitTime; 	  // !RVDB used to store the retryWaitTime (ms) for multiple ACK Send loop
volatile uint16_t RFM69X_SessionKey::_respDelayTime; 	  // !RVDB used to store the Session KEY challenge response for slow remote nodes
//=============================================================================
// initialize() - Some extra initialisation before calling base class
//=============================================================================
bool RFM69X_SessionKey::initialize(uint8_t freqBand, uint8_t nodeID, uint8_t networkID)
{
  ...
  return RFM69X::initialize(freqBand, nodeID, networkID);// use base class to initialise everything else
}
...
//=============================================================================
void RFM69X_SessionKey::send(uint8_t toAddress, const void* buffer, uint8_t bufferSize, bool requestACK)
{ 
...
//=============================================================================
void RFM69X_SessionKey::sendWithSession(uint8_t toAddress, const void* buffer, uint8_t bufferSize, bool requestACK, uint16_t retryWaitTime) {
...
//=============================================================================
void RFM69X_SessionKey::sendACK(const void* buffer, uint8_t bufferSize) {
...
//=============================================================================
void RFM69X_SessionKey::sendFrame(uint8_t toAddress, const void* buffer, uint8_t bufferSize, bool requestACK, bool sendACK)
{
...
//=============================================================================
void RFM69X_SessionKey::sendFrame(uint8_t toAddress, const void* buffer, uint8_t bufferSize, bool requestACK, bool sendACK, bool sessionRequested, bool sessionIncluded)
 {
//  Serial.print("\n\r Send Frame; Request ACK is: "), Serial.print(requestACK);Serial.print (" - Send ACK is: "); Serial.print (sendACK); Serial.print (" - Session RQST is: "); Serial.print (sessionRequested);Serial.print (" - Session INC: "); Serial.println (sessionIncluded);
   _gotISR = false;   // Ensure pending received interrupt to be cleared before sending a frame(used for ESP32 compatibility)
...
//=============================================================================
void RFM69X_SessionKey::interruptHook(uint8_t CTLbyte) {
...
//=============================================================================
void RFM69X_SessionKey::receiveBegin() {	
  SESSION_KEY_INCLUDED = 0;
  SESSION_KEY_REQUESTED = 0;
  RFM69X::receiveBegin();
}
...
//=============================================================================
 bool RFM69X_SessionKey::receiveDone() {
...
//{
  ...
  RFM69X::interruptHandling(); // Added for ESP32 to handle SPI transfer
...
//=============================================================================
void RFM69X_SessionKey::useSessionKey(bool onOff) {
  _sessionKeyEnabled = onOff;
}
//=============================================================================
//  sessionKeyEnabled() - Check if session key support is enabled
//=============================================================================
bool RFM69X_SessionKey::sessionKeyEnabled() {
...
//=============================================================================
void RFM69X_SessionKey::useSession3Acks(bool onOff) {
...
//=============================================================================
bool RFM69X_SessionKey::session3AcksEnabled() {
...
//=============================================================================
void RFM69X_SessionKey::sessionWaitTime(uint16_t waitTime) {
...
//=============================================================================
void RFM69X_SessionKey::sessionRespDelayTime(uint16_t respDelayTime) {
  if (respDelayTime > 1000) _respDelayTime = 1000;				// if the value is 0 use the maximum one of 500us
